# Generic adapter — any harness with a system prompt

Inject the block below into the agent's system prompt (or highest-priority
instruction slot). It is self-contained: no tools, plugins, or file layout
beyond the agent's ability to write files in its workspace.
