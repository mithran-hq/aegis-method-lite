# Aegis Method block for CLAUDE.md

Append the block below to your project's `CLAUDE.md` (or your global
`~/.claude/CLAUDE.md`) to make every session in the project work under the
method. Alternative: install the skill in `skill/aegis-method/` by copying
it to `.claude/skills/aegis-method/` and invoke it per task.
